...
public class TestCMSimple {
	...
	@Test
	public void checkCustomTest() {
		CommunicationsMonitor comm = new CommunicationsMonitor();
		for(int i=0; i< 100; i++)
			comm.addCommunication(1,1,1);

		comm.createGraph();

		Assert.assertEquals(1,comm.getComputerMapping(1).size());

		comm = new CommunicationsMonitor();
		for(int i=0; i< 10; i++)
			comm.addCommunication(1,1,i);

		comm.createGraph();

		Assert.assertEquals(10,comm.getComputerMapping(1).size());


		comm =new CommunicationsMonitor();
		comm.addCommunication(1,2,4);
		comm.addCommunication(2,4,8);
		comm.addCommunication(2,5,8);
		comm.addCommunication(4,3,8);
